...
function active_nav_init(){
	var active_nav = $('.nav .active');
	var offset = active_nav.offset();
	var body_bg = $('img.vegas-background');
	var active_bg_ps = '-'+offset.left+'px '+offset.top+'px';
	active_nav.find('a').css( {'background-image' : 'url('+body_bg.attr('src')+')', 'background-position' : active_bg_ps, 'background-size' : body_bg.width()+'px' });
